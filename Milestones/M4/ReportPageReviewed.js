...
const getPatientById = (id) => {
  ...
  return data.filter((patient) => {
    return patient.id.toString() === id; //REVIEW: uses === over ==, good
  });
};
...
class ReportPage extends Component {
  ...
  render() {
    ...
    var currentProgress = []; //REVIEW: good var declarations
    var maxProgress = []; 
    ...
    getProgressById(this.props.match.params.id, this.state.progressData).map(
      (chosen) => ( //REVIEW: proper use of arrow functions
      ...
      <div>
        ...
        <Row>
          <Col>
            ...
            {
              //to update numInProgress for ProfileCard to use
            }
            <ProfileCard
              id={this.state.id}
              ...
              progress={this.finalProgressPercentage(
                this.state.allProgressPercentage
              )}
